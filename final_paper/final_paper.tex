...
\subsection{Leaky Integrate-and-Fire approximation}

Instead of simulating the full time domain of every neuron, we consider the voltage at discrete (adjustable) timesteps. A neuron that fails to fire will have its potential ``leak,'' and a neuron that spikes will first have a high positive action potential, and then a negative potential that will ``leak'' back to the rest state. To simplify computations, we shift the system to have a rest state of 0V, as other approximations that we make in section \ref{TPO} will let us save a lot of memory by not storing data for any neuron where the membrane potential is 0 and there are no inbound signals.

Importantly, We do not store every instant in the time domain. Instead, all incoming signals are stored in a sorted container and processed in the order in which they arrive.
...
on every timestep $t$:
\vspace{5mm} \\
loop through every neuron $x$ for which the soonest spike is arriving at $t$
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item For every edge $e \in E_x$, send a spike to the target neuron, adjusted in amplitude and time of arrival by the data stored in $e$.
    ...
    \end{itemize}
\end{itemize}
Additionally, update the input neurons in $K_i$, and record anything from the output neurons in $K_o$
