...
public:
	...
	bin_arr calc(bin_arr input_layer_data)
	{
		//set input
	...
	void set_input(bin_arr input)
	{
		...
		{
			//TODO
			//check that bin_arr.size and layers[0].size are the same
			//need to write exception

			//push node if val at i is true
			if (input[i])
			{
				(layers[0])[i].add();
			}
	...
	bin_arr read_output()
	{
		...
		bin_arr output = bin_arr(fin.size);

		//store output values of final layer of nodes
		for (int i = 0; i < fin.size; i++)
		{
			//get first bit of data from node
			//copy into binary array
			ouput.set(i, fin[i].get_val());
		}
	}
